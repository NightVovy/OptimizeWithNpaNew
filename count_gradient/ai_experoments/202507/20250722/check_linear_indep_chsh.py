...
def calculate_parameters(theta, a0, a1, b0, b1):
    ...
    mea_B1 = np.cos(b1) * Z + np.sin(b1) * X  # caution
...
if __name__ == "__main__":
    # Set parameters
    theta = np.pi / 4  # Changed from pi/6 to pi/4
    a0 = 0
    a1 = np.pi / 2
    b0 = np.pi / 4  # Added
    b1 = -np.pi / 4  # Added
    ...
    params = calculate_parameters(theta, a0, a1, b0, b1)  # Modified to include b0 and b1
