...
def randomize_a0(a0, a1, iteration, max_initial_change=0.1):
    """Randomly change a0 with increasing amplitude but |a0| <= |a1|"""
    # Calculate maximum possible change that keeps |a0| <= |a1|
    max_possible_change = abs(a1) - abs(a0) if abs(a0) < abs(a1) else 0

    # Increase change amplitude with each iteration
    max_change = min(max_initial_change * (iteration + 1), max_possible_change)

    # Generate a random change within ±max_change
    change = random.uniform(-max_change, max_change)
    new_a0 = a0 + change

    # Ensure new_a0 doesn't exceed a1 in absolute value
    max_abs = abs(a1)
    if abs(new_a0) > max_abs:
        new_a0 = np.sign(new_a0) * max_abs

    return new_a0


def run_simulation(theta, a0, a1, iterations=5):
    """Run the simulation with increasing random a0 changes"""
    for i in range(iterations):
        print(f"\n=== Iteration {i + 1} ===")
        print(f"Current a0: {a0:.6f}")
        ...
        # Randomize a0 for next iteration with increasing amplitude
        a0 = randomize_a0(a0, a1, i)
        print(f"\nRandomized a0 for next iteration: {a0:.6f}")
...
# Run the simulation with 5 iterations
