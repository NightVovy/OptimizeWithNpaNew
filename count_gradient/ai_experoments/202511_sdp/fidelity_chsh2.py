...
if __name__ == '__main__':
    ...
    # --- 2. 运行 Test Case 1: Tsirelson Bound ---
    test_val_1 = 2 * np.sqrt(2)
    print(f"\n=== Test Case 1: CHSH Value = {test_val_1:.4f} (Tsirelson) ===")
    result_1 = fidelity_chsh(test_val_1, M_fc_input, desc_input, k=3)

    if result_1 is not None:
        print(f"Final Result (Minimized func): {result_1:.8f}")
    else:
        print("Result: None")

    # --- 3. 运行 Test Case 2: Local Bound ---
    test_val_2 = 2
    print(f"\n=== Test Case 2: CHSH Value = {test_val_2:.4f} (Local) ===")
    result_2 = fidelity_chsh(test_val_2, M_fc_input, desc_input, k=3)

    if result_2 is not None:
        print(f"Final Result (Minimized func): {result_2:.8f}")
    else:
        print("Result: None")