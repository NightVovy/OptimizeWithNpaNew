...
def compute_bell_limits(coefficients, desc, notation='cg', k=1, solver=None, verbose=False):
    """
    [新增功能]
    封装函数：同时计算 Bell 不等式的经典最大值 (Classical) 和量子最大值 (Quantum)。

    参数:
        coefficients: Bell 不等式系数
        desc: 场景描述 [OA, OB, MA, MB]
        notation: 'cg' (Collins-Gisin) 或 'fc' (Full Correlator)
        k: NPA 层级 (仅影响量子部分)
        solver: 指定求解器
        verbose: 打印调试信息

    返回:
        (b_cl, b_qm): 包含经典最大值和量子最大值的元组
    """
    # 1. 计算经典最大值
    b_cl = bell_inequality_max(
        coefficients, desc, notation=notation, mtype='classical',
        solver=solver, verbose=verbose
    )

    # 2. 计算量子最大值
    b_qm = bell_inequality_max(
        coefficients, desc, notation=notation, mtype='quantum',
        k=k, solver=solver, verbose=verbose
    )
...
if __name__ == '__main__':
    print("=== 测试 BellInequalityMax (compute_bell_limits 接口) ===")
    ...
    try:
        # 使用新增的接口同时获取两个值
        b_cl, b_qm = compute_bell_limits(M_fc, desc_chsh, notation='fc', k=3)

        print(f"\n[Classical Max]: {b_cl}")
        print(f"[Quantum Max (NPA k=3)]: {b_qm}")
