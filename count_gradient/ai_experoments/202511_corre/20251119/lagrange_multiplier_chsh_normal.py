import numpy as np
import chsh_normal_maxviolation as chsh_calc
...
def constraint_equation(p, s, t, print_terms=False):
    ...
    if print_terms:
        print(f"\n--- 约束方程 (s={s}, t={t}) ---")
        print(f"Term1 (A0B0): {term1:.8f} ({term1 / pi:.4f} * pi)")
        print(f"Term2 (A1B0): {term2:.8f} ({term2 / pi:.4f} * pi)")
        print(f"Term3 (A0B1): {term3:.8f} ({term3 / pi:.4f} * pi)")
        print(f"Term4 (A1B1): {term4:.8f} ({term4 / pi:.4f} * pi)")
        print(f"总和 (Term1+Term2-Term3+Term4): {(term1 + term2 - term3 + term4):.8f}")
    # <--- 新增打印逻辑 END --->
...
# 给定的点p
# p = chsh_calc.get_chsh_correlations()
p = np.array([
    0,  # A0
    0,  # A1
    0,  # B0
...
for s, t in all_combinations:
    # 调用 constraint_equation 并设置 print_terms=True 来触发打印
    # 打印结果会在函数内部输出
    constraint_equation(p, s, t, print_terms=True)
