...
try:
    ...
    print("请确保 NPAHierarchy_Custom.py, locate_gamma_2.py, BellInequalityMax_new.py 等文件在正确位置。")
    sys.exit(1)
...
def main():
    ...
    # Tilted-CHSH 系数
    # [Task 2] 保持原有的 convert_vector_to_matrix，因为它生成的矩阵 coeff_matrix
    # 结构正好符合 Full Correlator (FC) 格式: Row0=[?, B0, B1], Col0=[?, A0, A1], M[1,1]=A0B0...
    vec_ineq_rotated = [alpha_val, 0, 0, 0, 1, 1, 1, -1]
    coeff_matrix = convert_vector_to_matrix(vec_ineq_rotated)

    # [Task 1] 调用接口获取 Bounds
    print("[Info] Calculating Classical and Quantum bounds...")
    # 注意: 这里使用 NPA k=3 (L3) 类似于原代码中使用的硬编码量子界精度
    ...
    for idx, val in enumerate(target_vals):
        ...
        # [Task 3] 计算归一化违背值 V
        if quantum_bound - local_bound != 0:
            V_norm = (val - local_bound) / (quantum_bound - local_bound)
        else:
            V_norm = 0.0
        ...
        try:
            ...
            print(f"  [{idx + 1}/{num_points}] Val={val:.4f} (V={V_norm:.4f}) -> Solver Error")
        ...
        if prob.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
            ...
            results_val.append(val)
            # [Task 3] 输出信息中加入 V
            print(f"  [{idx + 1}/{num_points}] Val={val:.4f} (V={V_norm:.4f}) -> Fidelity={fid:.6f}")
        else:
            print(f"  [{idx + 1}/{num_points}] Val={val:.4f} (V={V_norm:.4f}) -> Status: {prob.status}")
            results_fid.append(None)
            results_val.append(val)
    ...
    x_raw, y_plot = zip(*valid_data)

    # [Task 3] 转换横轴为 V
    denom = quantum_bound - local_bound
    if denom == 0:
        x_plot = list(x_raw)
        xlabel_str = 'Tilted-CHSH Value (Bounds are equal)'
    else:
        x_plot = [(val - local_bound) / denom for val in x_raw]
        xlabel_str = 'Normalized Violation V = (Val - Local) / (Quantum - Local)'
    ...
    plt.title(f'Fidelity vs Normalized Violation (Alpha={alpha_val})')
    plt.grid(True)
    plt.legend()

    # 标记边界 (在归一化坐标下，Local=0, Quantum=1)
    if denom != 0:
        plt.axvline(x=0.0, color='r', linestyle='--', alpha=0.5, label='Local Bound (0)')
        plt.axvline(x=1.0, color='g', linestyle='--', alpha=0.5, label='Quantum Bound (1)')
    else:
        plt.axvline(x=local_bound, color='r', linestyle='--', alpha=0.5, label='Bound')
    ...
    # 保存图片
    save_path = os.path.join(fig_dir, 'tilted_alpha_figure2.png')
