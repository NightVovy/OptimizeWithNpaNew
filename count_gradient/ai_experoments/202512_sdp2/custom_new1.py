...
try:
    ...
    # 如果 split_projector.py 在子文件夹中，可能需要添加路径或调整导入
    # 这里假设它就在当前搜索路径下（或者大文件夹下平铺）
...
def convert_vector_to_matrix(vec):
    ...
    # 注意：这里的矩阵索引仅用于 coeff_matrix 的内部逻辑，与 Gamma 索引不同
    M = np.zeros((3, 3))
...
def build_fidelity_objective_from_json(G, ind_catalog, m_vec, theta_val, mu_val):
    ...
    for term_key, coeff_str in data.items():
        ...
        if gamma_elem is None:
            # 如果找不到，说明基底不够大，无法覆盖此项
            # 在本任务中，应确保 custom_basis 足够覆盖 json 中的项
        ...
        # 记录调试信息 (如果是 Variable 只能记录对象，求解后查看 value)
...
def build_tilted_chsh_constraint(G, ind_catalog, m_vec, coeff_matrix, target_val):
    ...
    # coeff_matrix 结构:
    # [0,0]=Const(通常0), [0,1]=B0, [0,2]=B1
    # [1,0]=A0, [1,1]=A0B0, [1,2]=A0B1
    # [2,0]=A1, [2,1]=A1B0, [2,2]=A1B1

    # 1. 常数项 (如果有)
    # 本题中 vec 只有 A, B, AB 项，没有纯常数

    # 定义查找辅助函数
    def get_E(alice_list, bob_list):
        # 注意：Bob 的序列在输入 get_gamma_element 时需要是全局索引 (2, 3)
        # 这里 Bob 的 list 输入也是 [2] 或 [3]
    ...
    # y=0 -> B0 (index 2), y=1 -> B1 (index 3)
    for y_local in range(2):
        c = coeff_matrix[0, y_local + 1]  # col 1, 2
        if c != 0:
            y_global = m_vec[0] + y_local  # 2 + 0/1
            term_Fy = get_E([], [y_global])
            # E[B_y] = 2 E[F_y] - 1 (因为 E[I]=1)
            expr += c * (2 * term_Fy - 1)

    # 3. Alice 的单体项 (Marginals) -> A_x = 2 E_x - I
    # x=0 -> A0 (index 0), x=1 -> A1 (index 1)
...
# ==============================================================================
def main():
    print("=== Custom NPA SDP for Fidelity with Tilted-CHSH (SWAP Method) ===")
    ...
    # 求解 alpha: a = 2 * cos(2theta) / sqrt(1 + sin(2theta)^2)
    alpha_val = 2 * cos_2theta / np.sqrt(1 + sin_2theta ** 2)

    print(f"[Params] Theta = {theta:.6f} (rad)")
    print(f"[Params] Alpha = {alpha_val:.6f}")
    print(f"[Params] Mu    = {mu:.6f} (rad)")
    ...
    # 调用 NPA 构建器
    # dummy_cg 用于占位，因为我们不 enforcing data from cg, 而是用约束
    ...
    G, constraints, ind_catalog = npa_constraints(
        ...
        enforce_data=False  # 不强制等于 dummy_cg
    )
    ...
    # 4. 构建目标函数 (Minimize Fidelity Func)
    # ==========================
    try:
    ...
    if prob.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
        ...
        for k in sorted_keys:
            ...
            # 格式化输出: Term | Coeff | Expectation | Contribution
