...
def analyze_linear_independence_with_values():
    ...
    # 使用最稳定的方法
    try:
        independent_indices = find_independent_stepwise(function_matrix, rank)
        print(f"\n方法1 - 逐步构建法找到的线性无关公式索引: {independent_indices}")
    except:
        # 如果方法1失败，使用方法2
        independent_indices = find_independent_by_svd(function_matrix, rank)
        print(f"\n方法2 - SVD法找到的线性无关公式索引: {independent_indices}")
